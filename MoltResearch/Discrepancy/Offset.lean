...
/-- Offset sum over a concatenated length equals the sum of two offset sums. -/
lemma apSumOffset_add_length (f : ℕ → ℤ) (d m n₁ n₂ : ℕ) :
    apSumOffset f d m (n₁ + n₂) = apSumOffset f d m n₁ + apSumOffset f d (m + n₁) n₂ := by
  -- rewrite everything as differences of `apSum`; the statement telescopes
  simp [apSumOffset_eq_sub, Nat.add_assoc]

/-- Offset sum of the pointwise sum of two functions equals the sum of their offset sums. -/
...
/-- Offset sum of the negation of a function equals the negation of its offset sum. -/
...
/-- Offset sum of the pointwise subtraction of two functions equals the subtraction of their offset sums. -/
