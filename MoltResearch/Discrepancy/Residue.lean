...
Checklist item: Problems/erdos_discrepancy.md (Track B) —
`apSumFrom`/`discOffset` mixed normal form for affine-residue splits.

This is a paper-friendly one-liner wrapper:
1. normalize an affine interval sum (`Finset.Icc` notation) directly to the nucleus
   `discOffset (fun k => f (a + k)) …` normal form; then
2. apply the residue-class split triangle bound.

The goal is that callers can write the standard “split into `q` progressions” step without
manual intermediate rewrites.
-/
lemma natAbs_sum_Icc_add_affineEndpoints_mul_len_succ_le_sum_range_natAbs
    (f : ℕ → ℤ) (a d m q n : ℕ) (hq : q > 0) :
    Int.natAbs ((Finset.Icc (m + 1) (m + q * (n + 1))).sum (fun i => f (a + i * d))) ≤
      (Finset.range q).sum (fun r =>
        Int.natAbs (f (a + (m + r + 1) * d) + apSumFrom f (a + (m + r + 1) * d) (q * d) n)) := by
  -- Step 1: paper interval sum → nucleus `discOffset`.
  have hnorm :
      Int.natAbs ((Finset.Icc (m + 1) (m + q * (n + 1))).sum (fun i => f (a + i * d))) =
        discOffset (fun k => f (a + k)) d m (q * (n + 1)) := by
    simpa using
      (natAbs_sum_Icc_add_affineEndpoints_eq_discOffset
        (f := f) (a := a) (d := d) (m := m) (n := q * (n + 1)))

  -- Step 2: apply the residue-class triangle bound at the nucleus level.
  have hbound :
      discOffset (fun k => f (a + k)) d m (q * (n + 1)) ≤
        (Finset.range q).sum (fun r =>
          Int.natAbs
            ((fun k => f (a + k)) ((m + r + 1) * d) +
              apSumFrom (fun k => f (a + k)) ((m + r + 1) * d) (q * d) n)) :=
    discOffset_mul_len_succ_le_sum_range_natAbs
      (f := fun k => f (a + k)) (d := d) (m := m) (q := q) (n := n) hq

  -- Step 3: rewrite the shifted `apSumFrom` back to the paper-friendly `a + …` form.
  have hrewrite :
      (Finset.range q).sum (fun r =>
          Int.natAbs
            ((fun k => f (a + k)) ((m + r + 1) * d) +
    ...
    refine Finset.sum_congr rfl ?_
    intro r hr
    -- `apSumFrom_shift_add_left` pushes the translation into the affine start.
    simp [apSumFrom_shift_add_left, Nat.add_assoc, Nat.add_left_comm, Nat.add_comm]

  -- Use `hnorm` as a rewrite, then `hbound`, then `hrewrite`.
  -- (Written as `calc` to keep the rewriting steps explicit and stable.)
  calc
    Int.natAbs ((Finset.Icc (m + 1) (m + q * (n + 1))).sum (fun i => f (a + i * d)))
        = discOffset (fun k => f (a + k)) d m (q * (n + 1)) := hnorm
    _ ≤ (Finset.range q).sum (fun r =>
          Int.natAbs
            ((fun k => f (a + k)) ((m + r + 1) * d) +
              apSumFrom (fun k => f (a + k)) ((m + r + 1) * d) (q * d) n)) := hbound
    _ = (Finset.range q).sum (fun r =>
          Int.natAbs (f (a + (m + r + 1) * d) + apSumFrom f (a + (m + r + 1) * d) (q * d) n)) := by
            simpa [hrewrite]
