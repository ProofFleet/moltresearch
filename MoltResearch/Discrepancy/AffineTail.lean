...
lemma apSumFrom_tail_eq_apSumOffset_shift_add (f : ℕ → ℤ) (a d m n : ℕ) :
  ...
  -- Direct bridge: avoid commuting/reassociating arithmetic under binders.
  simpa using
...
lemma apSumFrom_tail_eq_apSumOffset_shift_add_mul_left (f : ℕ → ℤ) (a d m n : ℕ) :
  ...
  -- Direct bridge (mul-left friendly).
  simpa using
    (apSumFrom_add_mul_left_eq_apSumOffset_shift_add (f := f) (a := a) (d := d) (m := m) (n := n))
...
lemma apSumFrom_tail_succ_length_eq_add_apSumOffset_shift (f : ℕ → ℤ) (a d m n : ℕ) :
  ...
  calc
    ...
    _ = f (a + (m + 1) * d) + apSumOffset (fun k => f (a + k)) d (m + 1) n := by
          -- `simp` keeps affine endpoints in the `a + (m+1)*d` shape (via `add_mul_succ_norm`),
          -- so this tail→offset rewrite matches without extra commutativity noise.
          simpa using
...
These convenience wrappers were redundant with the canonical step-one lemmas in `Affine.lean`.
They have been moved to `MoltResearch.Discrepancy.Deprecated` as deprecated aliases so the stable
surface exports exactly one preferred name for this normal form.
-/
...
/-- Normal form (paper → nucleus, tail, glue, fixed-length, translation-friendly): rewrite the
affine interval sum `∑ i ∈ Icc (m+1) (m+n), f (a + i*d)` directly as an offset AP sum on the
shifted sequence `k ↦ f (k + a)`.

This is the “paper boundary” bridge lemma: it combines `sum_Icc_eq_apSumFrom_tail` with
`apSumFrom_tail_eq_apSumOffset_shift_add` so downstream work can normalize paper tails into the
translation-friendly `apSumOffset (fun k => f (k + a)) …` nucleus API without `Nat.add_comm` churn.
-/
lemma sum_Icc_eq_apSumOffset_shift_add_left (f : ℕ → ℤ) (a d m n : ℕ) :
    (Finset.Icc (m + 1) (m + n)).sum (fun i => f (a + i * d)) =
      apSumOffset (fun k => f (k + a)) d m n := by
  calc
    (Finset.Icc (m + 1) (m + n)).sum (fun i => f (a + i * d)) = apSumFrom f (a + m * d) d n := by
...
/-!
### Endpoint-normalization aliases producing `apSumOffset (fun k => f (a + k)) …`

The existing endpoint-normalization lemmas in this file typically produce the shifted-sequence form
`apSumOffset (fun k => f (k + a)) …`.

The following aliases expose the equivalent but sometimes more convenient binder shape
`fun k => f (a + k)`, so downstream callers can avoid `Nat.add_comm` rewrites under lambdas.
-/

/-- Endpoint-normalization (`m ≤ n`) for affine tails written as `f (a + i*d)`, producing the
shifted-sequence offset sum with binder `fun k => f (a + k)`.

This is a thin wrapper around `sum_Icc_eq_apSumOffset_shift_add_of_le_left`.
-/
lemma sum_Icc_eq_apSumOffset_of_le_affine_left (f : ℕ → ℤ) (a d : ℕ) {m n : ℕ} (hmn : m ≤ n) :
    (Finset.Icc (m + 1) n).sum (fun i => f (a + i * d)) =
      apSumOffset (fun k => f (a + k)) d m (n - m) := by
  simpa [Nat.add_comm] using
    (sum_Icc_eq_apSumOffset_shift_add_of_le_left (f := f) (a := a) (d := d) (m := m) (n := n)
      (hmn := hmn))

/-- Mul-left variant of `sum_Icc_eq_apSumOffset_of_le_affine_left`, with summand written as
`f (a + d*i)`.
-/
lemma sum_Icc_eq_apSumOffset_of_le_affine_left_mul_left (f : ℕ → ℤ) (a d : ℕ) {m n : ℕ}
    (hmn : m ≤ n) :
    (Finset.Icc (m + 1) n).sum (fun i => f (a + d * i)) =
      apSumOffset (fun k => f (a + k)) d m (n - m) := by
...
/-- Alias for `sum_Icc_eq_apSumOffset_shift_add_of_le_left` (same statement, naming aligned with
`sum_Icc_eq_apSumOffset_of_le`). -/
lemma sum_Icc_eq_apSumOffset_of_le_shift_add_left (f : ℕ → ℤ) (a d : ℕ) {m n : ℕ}
    (hmn : m ≤ n) :
    (Finset.Icc (m + 1) n).sum (fun i => f (a + i * d)) =
      apSumOffset (fun k => f (k + a)) d m (n - m) := by
  simpa using
...
/-- Mul-left alias for `sum_Icc_eq_apSumOffset_shift_add_of_le_left_mul_left`. -/
lemma sum_Icc_eq_apSumOffset_of_le_shift_add_left_mul_left (f : ℕ → ℤ) (a d : ℕ) {m n : ℕ}
    (hmn : m ≤ n) :
    (Finset.Icc (m + 1) n).sum (fun i => f (a + d * i)) =
      apSumOffset (fun k => f (k + a)) d m (n - m) := by
  simpa using
...
/-!
### Endpoint-normalization aliases (affine endpoints)

Downstream callers often think in terms of the *values* being summed, whose endpoints are
`a + (m+1)*d` and `a + n*d`. The following thin wrappers provide `…_of_le…` names for the common
summand shapes `a + i*d` / `a + d*i` while producing the `fun k => f (k + a)` binder form.
-/

/-- Endpoint-normalization (`m ≤ n`) for affine tails in the common `a + i*d` summand shape.

This is an alias for `sum_Icc_eq_apSumOffset_of_le_shift_add_left`. -/
lemma sum_Icc_eq_apSumOffset_of_le_affine (f : ℕ → ℤ) (a d : ℕ) {m n : ℕ} (hmn : m ≤ n) :
    (Finset.Icc (m + 1) n).sum (fun i => f (a + i * d)) =
      apSumOffset (fun k => f (k + a)) d m (n - m) := by
  simpa using
    (sum_Icc_eq_apSumOffset_of_le_shift_add_left (f := f) (a := a) (d := d) (m := m) (n := n)
      (hmn := hmn))

/-- Mul-left variant of `sum_Icc_eq_apSumOffset_of_le_affine`, with summand `a + d*i`.

This is an alias for `sum_Icc_eq_apSumOffset_of_le_shift_add_left_mul_left`. -/
lemma sum_Icc_eq_apSumOffset_of_le_affine_mul_left (f : ℕ → ℤ) (a d : ℕ) {m n : ℕ}
    (hmn : m ≤ n) :
    (Finset.Icc (m + 1) n).sum (fun i => f (a + d * i)) =
      apSumOffset (fun k => f (k + a)) d m (n - m) := by
  simpa using
    (sum_Icc_eq_apSumOffset_of_le_shift_add_left_mul_left (f := f) (a := a) (d := d) (m := m)
      (n := n) (hmn := hmn))
